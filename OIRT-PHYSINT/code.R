...
setwd("/Users/dag/github/hrps_v1")
#----------------------------------------------------------------------------------------------------#
# CIRI Physint data  1981-2010
ciri <- read.csv("./data/CIRI_physint_data20120401.csv", na.strings=c(-999, -66, -77))
year.start <- 1981
...
#data <- na.omit(subset(data, select=c(CTRY, YEAR, CIRI, COW, DISAP, KILL, POLPRIS, TORT)))
data <- subset(data, !is.na(DISAP) | !is.na(KILL) | !is.na(POLPRIS) | !is.na(TORT))
n <- nrow(data)
n
...
# full model (4 items)
y.pre <- as.matrix(data[,c(5, 6, 7, 8)])
...
dump("country", "./alldraws/oirtphysint-country.R")
dump("year", "./alldraws/oirtphysint-year.R")
...
inits.function <- function(chain){
  return(
    switch(
      ...
      "1" = list(
        mu = matrix(
          rnorm(length(panel) * max(panel), mean = 0, sd = 1),
          nrow = length(panel),
          ncol=max(panel)
        ),
        beta = runif(4),
        alpha0 = matrix(
          c(runif(4), runif(4, 1, 2)),
          nrow = 4, ncol = 2, byrow = TRUE)
      ),
      "2" = list(
        mu = matrix(
          rnorm(length(panel) * max(panel), mean = 0, sd = 1),
          nrow = length(panel), ncol = max(panel)
        ),
        beta = runif(4),
        alpha0 = matrix(
          c(runif(4), runif(4, 1, 2)),
...
#rjags code version
m <- jags.model(
  file = "./OIRT-PHYSINT/model.bug",
  data = list(
    "y" = y, "year" = year, "country" = country,
    "n.country" = length(panel),
    "n.year" = max(panel),
    "n"=nrow(y)
  ),
  inits = inits.function, n.chains = 3, n.adapt = ADAPT
)
update(m, BURNIN)
